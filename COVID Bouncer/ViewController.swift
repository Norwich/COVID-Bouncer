...
class ViewController: UIViewController, UINavigationControllerDelegate {
    ...
    lazy var detectionRequest: VNCoreMLRequest = {
        do {
            let model = try VNCoreMLModel(for: COVIDBouncer().model)
    ...
    func drawDetectionsOnPreview(detections: [VNRecognizedObjectObservation]) {
        ...
        for detection in detections {
            let topLabelObservation = detection.labels[0]
            if topLabelObservation.confidence >= 0.9 {
                //let objectBounds = VNImageRectForNormalizedRect(detection.boundingBox, Int(bufferSize.width), Int(bufferSize.height))
                ...
                if topLabelObservation.identifier == "with_mask" {
